...
%                 title('mean of events1 on events 2 damain');
                ...
                for ii = 1:length(events_cell)
                    ...
                    c_data_for_classifier{ii} = covFromCellArrayOfEvents( events_cell{ii} );
...
function [pre1, pre2] = calPrecision(X, X_test,y, y_test, data_name)
    ...
    class1            = 1;
    class2            = 2;
    class1_cov        = X(:,:,vClass == class1);
    class2_cov        = X(:,:,vClass == class2);
    ...
    dist1 = calcDistanceBetweenCovMat(mean1, X_test);
    dist2 = calcDistanceBetweenCovMat(mean2, X_test);
    
    predicted_label = (dist1 > dist2) + 1;
